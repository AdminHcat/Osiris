...
void GUI::renderAimbotWindow(Config& config, bool contentOnly) noexcept
{
    ...
    ImGui::Checkbox("按键", &config.aimbotOnKey);
    ImGui::SameLine();
    ...
    ImGui::PushID(0);
    ImGui::Combo("", &currentCategory, "全部\0手枪\0重型\0冲锋枪\0步枪\0");
    ImGui::PopID();
    ImGui::SameLine();
    ...
    ImGui::Checkbox("友军火力", &config.aimbot[currentWeapon].friendlyFire);
    ImGui::Checkbox("仅可视", &config.aimbot[currentWeapon].visibleOnly);
    ...
    ImGui::Checkbox("自动射击", &config.aimbot[currentWeapon].autoShot);
    ImGui::Checkbox("自动开镜", &config.aimbot[currentWeapon].autoScope);
    ImGui::Combo("Bone", &config.aimbot[currentWeapon].bone, "最近\0最佳伤害\0头部\0脖子\0胸骨\0胸部\0胃部\0盆骨\0");
    ImGui::NextColumn();
...
void GUI::renderTriggerbotWindow(Config& config, bool contentOnly) noexcept
{
    ...
    ImGui::Checkbox("友军火力", &config.triggerbot[currentWeapon].friendlyFire);
...
void GUI::renderStyleWindow(Config& config, bool contentOnly) noexcept
{
    ...
    if (ImGui::Combo("菜单样式", &config.style.menuStyle, "经典\0单窗口\0"))
        window = { };
    if (ImGui::Combo("Menu colors", &config.style.menuColors, "暗色\0亮色\0经典\0自定义\0"))
        updateColors(config);
    ImGui::PopItemWidth();
...
void GUI::renderConfigWindow(const OtherInterfaces& interfaces, const Memory& memory, Config& config, bool contentOnly) noexcept
{
    ...
    if (ImGui::ListBox("", &currentConfig, [](void* data, int idx, const char** out_text) {
        ...
        *out_text = (const char*)vector[idx].c_str();
        ...
        if (currentConfig != -1) {
            if (ImGui::Button("加载所选", { 100.0f, 25.0f })) {
            ...
            if (ImGui::Button("保存所选", { 100.0f, 25.0f }))
                config.save(interfaces, memory, currentConfig);
            if (ImGui::Button("删除所选", { 100.0f, 25.0f })) {
