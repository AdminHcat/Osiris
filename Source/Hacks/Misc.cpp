...
void Misc::watermark() noexcept
{
    ...
    ImGui::Begin("水印", nullptr, windowFlags);
...
void Misc::drawBombTimer() noexcept
{
    ...
    std::ostringstream ss; ss << "炸弹位于 " << (!plantedC4.bombsite ? 'A' : 'B') << "点 : " << std::fixed << std::showpoint << std::setprecision(3) << (std::max)(plantedC4.blowTime - memory.globalVars->currenttime, 0.0f) << " 秒";
    ...
    if (plantedC4.defuserHandle != -1) {
        ...
        if (plantedC4.defuserHandle == GameData::local().handle) {
            if (canDefuse) {
                ...
                ImGui::textUnformattedCentered("你无法拆除！");
            }
            ImGui::PopStyleColor();
        } else if (const auto defusingPlayer = GameData::playerByHandle(plantedC4.defuserHandle)) {
            std::ostringstream ss; ss << defusingPlayer->name << " 正在拆除: " << std::fixed << std::showpoint << std::setprecision(3) << (std::max)(plantedC4.defuseCountDown - memory.globalVars->currenttime, 0.0f) << " 秒";
            ImGui::textUnformattedCentered(ss.str().c_str());
...
void Misc::drawGUI(Visuals& visuals, inventory_changer::InventoryChanger& inventoryChanger, Glow& glow, bool contentOnly) noexcept
{
    ...
    ImGui::hotkey("菜单键", miscConfig.menuKey);
    ...
    ImGui::Checkbox("自动重载", &miscConfig.autoReload);
    ...
    if (ImGui::BeginPopup("")) {
        ImGui::Checkbox("生命条", &miscConfig.offscreenEnemies.healthBar.enabled);
        ImGui::SameLine();
        ImGui::SetNextItemWidth(95.0f);
        ImGui::Combo("类型", &miscConfig.offscreenEnemies.healthBar.type, "Gradient\0Solid\0Health-based\0");
    ...
    ImGui::PopID();
    ImGui::Combo("命中音效", &miscConfig.hitSound, "无\0Metal\0Gamesense\0Bell\0Glass\0自定义\0");
    if (miscConfig.hitSound == 5) {
        ImGui::InputText("命中音效文件名", &miscConfig.customHitSound);
        if (ImGui::IsItemHovered())
            ImGui::SetTooltip("音频文件必须放在 csgo/sound/ 目录下");
    }
    ImGui::PushID(5);
    ImGui::Combo("击杀音效", &miscConfig.killSound, "无\0Metal\0Gamesense\0Bell\0Glass\0自定义\0");
    ...
    if (ImGui::BeginPopup("")) {
        ...
        ImGui::Combo("模式", &miscConfig.purchaseList.mode, "Details\0Summary\0");
    ...
    if (ImGui::BeginPopup("")) {
        ...
        ImGui::Combo("目标", &miscConfig.reportbot.target, "Enemies\0Allies\0All\0");
        ImGui::InputInt("延迟 (s)", &miscConfig.reportbot.delay);
