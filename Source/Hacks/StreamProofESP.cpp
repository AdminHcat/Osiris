...
void StreamProofESP::drawGUI(Config& config, bool contentOnly) noexcept
{
    ...
    if (ImGui::BeginListBox("##list", { 170.0f, 300.0f })) {
        ...
        for (std::size_t i = 0; i < categories.size(); ++i) {
            ...
            const auto items = [](std::size_t category) noexcept -> std::vector<const char*> {
                ...
                case 3: return { "闪光弹", "高爆手雷", "Breach Charge", "Bump Mine", "诱饵弹", "莫洛托夫", "TA Grenade", "烟雾弹", "雪球" };
            ...
            for (std::size_t j = 0; j < items.size(); ++j) {
                ...
                const auto subItems = [](std::size_t item) noexcept -> std::vector<const char*> {
                    ...
                    case 6: return { "闪光弹", "高爆手雷", "烟雾弹", "莫洛托夫", "诱饵弹", "燃烧弹", "TA Grenade", "Fire Bomb", "Diversion", "Frag Grenade", "雪球" };
    ...
    if (ImGui::BeginChild("##child", { 400.0f, 0.0f })) {
        ...
        ImGui::Combo("##1", &sharedConfig.snapline.type, "底部\0顶部\0Crosshair\0");
        ...
        if (currentCategory < 2) {
            ...
            ImGui::SameLine(spacing);
            ImGui::Checkbox("生命条", &playerConfig.healthBar.enabled);
            ImGui::SameLine();
