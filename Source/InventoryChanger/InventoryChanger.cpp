...
void InventoryChanger::drawGUI(const Memory& memory, bool contentOnly)
{
    ...
    if (isInAddMode) {
        ...
        const bool filterChanged = ImGui::InputTextWithHint("##search", "搜索武器皮肤、贴纸、刀具、手套、音乐盒..", &filter);
        ImGui::SameLine();
        const bool addingAll = ImGui::Button("添加列表中的所有内容");
